...
export class ProcessorService {
    ...
    compareRanges = async (firstRange: any[], secondRange: any[], inputParams: RangeComparisonInputParams, outputParams: OutputParams): Promise<ProcessorResult> => {
        return await this._comparator.compareRanges(firstRange, secondRange, inputParams, outputParams);
    }
    
    extractData = async (contextId: uuid, inputParams: DataExtractionInputParams, outputParams: OutputParams): Promise<ProcessorResult> => {
        return await this._extractor.extractData(contextId, inputParams, outputParams);
    }
    
    storeData = async (contextId: uuid, records: any[], inputParams: DataStorageInputParams, outputParams: OutputParams): Promise<ProcessorResult> => {
